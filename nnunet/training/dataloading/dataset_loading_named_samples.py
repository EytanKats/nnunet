...
def get_discretized_patch_id(case_shape, bbox_lubs):
    PATCH_DISCRETIZATION = 3

    patch_idx = 0
    for dim_idx, dim_len in enumerate(case_shape):
        patch_idx *= PATCH_DISCRETIZATION
        dim_points = np.linspace(0, dim_len, PATCH_DISCRETIZATION+2)[1:-1]
        patch_idx += np.argmin(np.abs(dim_points - np.mean(bbox_lubs[dim_idx])))
        # 2D: Will result to patch_idx = PATCH_DISCRETIZATION * X_NEAREST + Y_NEAREST
        # 3D: Will result to patch_idx = (PATCH_DISCRETIZATION * X_NEAREST + Y_NEAREST) * PATCH_DISCRETIZATION + Z_NEAREST

    return patch_idx

# def get_unique_class_id_lambda(case_properties, case_shape, bbox_lubs):
#     lmbd = lambda class_id: get_unique_sample_id(case_properties, case_shape, bbox_lubs) + f"_classid:{class_id}"
#     return lmbd
...
class NamedSampleDataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        class_id_lambda = get_unique_class_id_lambda(
            case_properties,
            shape,
            bbox_lubs
        )

        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'class_id_lambda':class_id_lambda}


class NamedSampleDataLoader2D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, oversample_foreground_percent=0.0,
        ...
        """
        super(NamedSampleDataLoader2D, self).__init__(data, batch_size, None)
        if pad_kwargs_data is None:
            pad_kwargs_data = OrderedDict()
        self.pad_kwargs_data = pad_kwargs_data
        self.pad_mode = pad_mode
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            #     bbox_lubs
            # )
            case_id = f"{i}_slc:{random_slice}_dpid:{get_discretized_patch_id(shape, bbox_lubs)}"
            all_case_ids.append(case_id)

        keys = selected_keys
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'case_ids':all_case_ids}
