...
def get_data_iterator_from_lists_of_filenames(input_list_of_lists: List[List[str]],
    ...
    num_processes = max(0, min(num_processes, len(input_list_of_lists)))
    num_processes_ppa = max(1, min(num_processes, len(input_list_of_lists)))
    ppa = PreprocessAdapter(input_list_of_lists, seg_from_prev_stage_files, preprocessor,
                            ...
                            configuration_manager, num_processes_ppa)
    if num_processes == 0:
