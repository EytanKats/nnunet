...
class nnUNetTrainer_MIND_GIN(nnUNetTrainer):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        def mind_gin_hook(module, input):
            input = input[0]
            B,_,D,H,W = input.shape

            input = MIND3D().forward(input)
            input = input.view(B*12,1,D,H,W)
            ...
            input = input.view(B,12,D,H,W)
            return input

        network.register_forward_pre_hook(mind_gin_hook)
