...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, use_compressed, disable_checkpointing, c, val, pretrained_weights, npz, world_size, epochs):
    setup_ddp(rank, world_size)
    torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, tr, p,
                                           use_compressed)
    nnunet_trainer.num_epochs = epochs
...
def run_training(args,
    ...
    if num_gpus > 1:
        ...
        if disable_mirroring:
            nnunet_trainer.disable_mirroring = disable_mirroring
